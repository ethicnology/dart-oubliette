...
void main() {
  ...
  group('slot separator is reserved', () {
    ...
    test('store rejects a separator-containing prefix from a named ctor',
        () async {
  ...
  group('decrypt trust boundary (#5 / M1)', () {
    test('fetch decrypts with the slot-derived aad and alias, not the blob',
      ...
      expect(call['aad'], buildSlot('oubliette_only_unlocked_', 'k'),
    ...
    test('fetch does not regenerate a key when the alias was cleared',
        () async {
      ...
      await s.fetch('k');
      expect(mock.aliases.contains('oubliette_only_unlocked'), isFalse,
    ...
    test('fetch throws PayloadTamperException on a relocated aad', () async {
      ...
      await prefs.setString(
          buildSlot('oubliette_only_unlocked_', 'k'), tampered.toJson());
    ...
    test('store rethrows key_invalidated and NEVER deletes the key', () async {
      ...
      await expectLater(
        ...
        throwsA(isA<KeyInvalidatedException>()
            .having((e) => e.keyAlias, 'keyAlias', 'oubliette_authenticated_fatal')),
      );
      expect(mock.deleteEntryCalls, 0,
          reason: 'the library must never delete a key on the user\'s behalf');
      expect(mock.aliases.contains('oubliette_authenticated_fatal'), isTrue,
  ...
  group('typed error mapping (M3): branch on recoverable, never mis-purge', () {
    ...
    test('decrypt_failed → DecryptionFailedException (not recoverable)',
        () async {
    ...
    test('auth_error → AuthenticationFailedException (the code Android emits)',
        () async {
    ...
    test('a malformed on-disk blob → PayloadCorruptException', () async {
      ...
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString(
          buildSlot('oubliette_only_unlocked_', 'k'), 'not valid json');
  ...
  group('per-key store lock (A4)', () {
    ...
    test('two SEPARATE instances racing the same slot: exactly one wins',
        () async {
      ...
      final b = storage();
      final results = await Future.wait([
        a.store('shared', Uint8List.fromList([1])).then((_) => 'ok').catchError((_) => 'err'),
        b.store('shared', Uint8List.fromList([2])).then((_) => 'ok').catchError((_) => 'err'),
      ]);
  ...
  group('purge (whole-profile destroy)', () {
    test('removes every blob in the profile and deletes the key', () async {
      ...
      expect(mock.aliases, isEmpty, reason: 'the shared profile key is deleted');
    ...
    test('only wipes its own prefix, leaving sibling profiles intact', () async {
      ...
      expect(mock.aliases, contains('oubliette_even_locked'),
    ...
    test('purging a custom profile does NOT wipe a nested custom sibling',
      ...
      AndroidOubliette custom(String prefix, String alias) => AndroidOubliette(
      ...
      expect(mock.aliases.contains('app_admin_key'), isTrue,
